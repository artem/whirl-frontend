...
struct RPCResponseMessage {
  ...
  SERIALIZE(CEREAL_NVP(request_id), CEREAL_NVP(result), CEREAL_NVP(error))
...
class RPCChannel : public IRPCChannelImpl, public INetSocketHandler {
 ...
 public:
  RPCChannel(Heap& h, ProcessNetwork& net, ServerAddress peer)
      : heap_(h), net_(net), peer_(peer), socket_(net_.ConnectTo(peer_, this)) {
  }
  ...
  // Context: Server
  Future<std::string> Call(const std::string& method,
                           const std::string& input) override {
    WHIRL_LLOG("Request method '" << method << " on peer " << peer_);

    ProcessSocket& socket = GetTransportSocket();
  ...
  void HandleLost() override {
    ...
    WHIRL_LLOG("Socket lost, fail all pending requests");
 ...
 private:
  ProcessSocket& GetTransportSocket() {
    ...
    WHIRL_LLOG("Invalid socket, reconnect to " << peer_);
...
class RPCClient : public IRPCClientImpl {
 public:
  RPCClient(Heap& h, ProcessNetwork& net) : heap_(h), net_(net) {
  }
...
class RPCServer : public INetSocketHandler, public IRPCServerImpl {
 public:
  RPCServer(Heap& heap, ProcessNetwork& net, IExecutorPtr executor)
  ...
  void RegisterMethod(const std::string& method,
    ...
    if (method_invokers_.find(method) != method_invokers_.end()) {
      WHEELS_PANIC("RPC method '" << method << "' already registered");
    }

    method_invokers_.emplace(method, std::move(invoker));
  }
 ...
 private:
  ...
  std::map<std::string, RPCMethodInvoker> method_invokers_;
