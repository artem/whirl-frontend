...
class LocalBytesStorage {
 public:
  Bytes Get(const Bytes& key) {
    ...
    if (found != data_.end()) {
      return found->second;  // string allocated in node's heap
    } else {
      return {};
    }
  }
...
class LocalKVStorageImpl : public ILocalStorageEngine {
 public:
  LocalKVStorageImpl(LocalBytesStorage& impl) : impl_(impl) {
  }
